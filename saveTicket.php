...
if (isset($_FILES['myFile'])) {
    ...
    $tid = $_GET['id'];
    $folder = "images/tickets/{$tid}";
    mkdir_recursive($folder);

    $file = $_FILES['myFile']['tmp_name']; 
    $fileNewName = $img_name;
    $folderPath = $folder."/";
    $imageurl = $folderPath.$fileNewName;

    if(move_uploaded_file($file, $imageurl)){

        $resizeObj = new imgEdit($imageurl);
        $resizeObj -> resizeImage(540, 800);
        $resizeObj -> saveImage($imageurl, 95);
    }
}
